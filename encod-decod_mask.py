...
# 
# Modified to support special tokens: <MASK>, <UNMASKED>, <START>, <END>
...
class CustomTokenizer:
    """
    Custom tokenizer wrapper that adds special tokens to tiktoken.
    Handles <MASK>, <UNMASKED>, <START>, <END>, and <PAD> tokens.
    """
    def __init__(self, base_tokenizer_name="gpt2"):
        self.base_tokenizer = tiktoken.get_encoding(base_tokenizer_name)
        
        # Define special tokens
        self.special_tokens = {
            "<MASK>": 50257,      # Use the next available token ID
            "<UNMASKED>": 50258,  # Use the next available token ID
            "<START>": 50259,     # Use the next available token ID
            "<END>": 50260,       # Use the next available token ID
            "<PAD>": 0            # Use 0 for padding (standard)
        }
        ...
        self.vocab_size = max(self.base_tokenizer.n_vocab, max(self.special_tokens.values()) + 1)
    
    def encode(self, text):
        """Encode text, handling special tokens."""
        # Split text into tokens, preserving special tokens
        tokens = []
        current_text = text
        
        # Handle special tokens
        for special_token in self.special_tokens.keys():
            if special_token in current_text:
                parts = current_text.split(special_token)
                for i, part in enumerate(parts):
                    if part:  # Add regular tokens
                        tokens.extend(self.base_tokenizer.encode(part))
                    if i < len(parts) - 1:  # Add special token (except for last part)
                        tokens.append(self.special_tokens[special_token])
                break
        else:
            # No special tokens found, use base tokenizer
            tokens = self.base_tokenizer.encode(current_text)
    ...
    def decode(self, token_ids):
        ...
        for token_id in token_ids:
            if token_id in self.id_to_token:
                # Special token
                text_parts.append(self.id_to_token[token_id])
            else:
                # Regular token
                text_parts.append(self.base_tokenizer.decode([token_id]))
...
def calc_loss_batch(src_batch, tgt_batch, model, device, pad_token_id=0):
    ...
    # Create mask for padding tokens and special tokens we don't want to predict
    mask = (targets_flat != pad_token_id) & (targets_flat != 50258)  # Exclude <UNMASKED> from loss
    
    # Calculate loss only on non-padded and non-special tokens
...
def generate_and_print_sample(model, src_tokenizer, tgt_tokenizer, device, start_context):
    ...
    with torch.no_grad():
        ...
        start_token_id = tgt_tokenizer.special_tokens["<START>"]
        ...
        for _ in range(100):  # Max length
            ...
            # Stop if end token
            end_token_id = tgt_tokenizer.special_tokens["<END>"]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train encoder/decoder transformer model with special tokens')
    parser.add_argument(
        '--mode',
        type=str,
    ...
    parser.add_argument(
        ...
        help='Input JSON file containing masked pre-training data'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
...
def main():
    ...
    # Setup tokenizers (using custom tokenizer with special tokens)
    try:
        src_tokenizer = CustomTokenizer("gpt2")
        tgt_tokenizer = CustomTokenizer("gpt2")
        logging.info("Successfully initialized custom tokenizers with special tokens")
        logging.info(f"Vocabulary size: {src_tokenizer.get_vocab_size()}")
    except Exception as e:
    ...
    # Model configuration
    BASE_CONFIG = {
        "src_vocab_size": src_tokenizer.get_vocab_size(),  # Use custom vocab size
        "tgt_vocab_size": tgt_tokenizer.get_vocab_size(),  # Use custom vocab size
    ...
    if args.mode == 'train':
        ...
        try:
            ...
            # Use masked context for sample generation
            new_train_losses, new_val_losses, new_tokens_seen = train_model(
                model, train_loader, val_loader, optimizer, device,
                num_epochs=num_epochs, eval_freq=10, eval_iter=5,
                start_context="<MASK>\n#2 = CARTESIAN_POINT('NONE',(484.40, 0.00, 0.00));\n#3 = CARTESIAN_POINT('NONE',(484.40, 128.80, 0.00));", 
            ...
            # Save model
            model_save_path = os.path.join(args.output_dir, f"transformer-{args.model_size}-masked-pretrained.pth")
            torch.save({
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'train_losses': train_losses,
